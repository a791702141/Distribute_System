...
// Make input file
func makeInputs(num int) []string { // used for generated files .. num is numbers of files  
	var names []string
	var i = 0
	for f := 0; f < num; f++ {
		names = append(names, fmt.Sprintf("824-mrinput-%d.txt", f))
		file, err := os.Create(names[f]) // Create a file and return fileinfo and error info 
		if err != nil { // nil is NULL
			log.Fatal("mkInput: ", err) //output error info 
		}
		w := bufio.NewWriter(file)// write file
...
func TestSequentialSingle(t *testing.T) { // test for sequent task
	mr := Sequential("test", makeInputs(1), 1, MapFunc, ReduceFunc)
	/* jobName Files nreduce mapF(func) reduceF(func)*/
	fmt.Printf("Test for single sequential \n")
	mr.Wait()
	check(t, mr.files)
...
func TestSequentialMany(t *testing.T) {
	...
	fmt.Printf("Test for many sequential \n")
	mr.Wait()
	check(t, mr.files)
	checkWorker(t, mr.stats)
	cleanup(mr)
