...
func Sequential(jobName string, files []string, nreduce int, // part 1 test func
	...
	go mr.run(jobName, files, nreduce, func(phase jobPhase) {
		...
		case reducePhase:
			for i := 0; i < mr.nReduce; i++ {
				//fmt.Printf("Start do Reduce")
				//fmt.Println(len(mr.files)) //1 5 5 5
...
func (mr *Master) run(jobName string, files []string, nreduce int,
	schedule func(phase jobPhase), // used for run 
	finish func(),
) {
	mr.jobName = jobName
	mr.files = files
